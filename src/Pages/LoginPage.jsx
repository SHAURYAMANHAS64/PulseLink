...
export default function LoginPage() {
  ...
  const handleLoginSubmit = (e) => {
    ...
    setTimeout(() => {
      ...
      localStorage.setItem(
        "user",
        JSON.stringify({
          name: formData.loginEmail.split("@")[0], // temp username
  ...
  // ⭐ SIGNUP HANDLER — ALSO STORES USER
  ...
  return (
    <div
      ...
      style={{
        backgroundImage:
          "linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)), url('/assets/gym.png')",
      }}
    >
      <div
        className={`w-full max-w-md bg-black/70 backdrop-blur-lg border border-white/10 rounded-2xl p-8 md:p-10 shadow-2xl transition-all duration-500 ${
        ...
        <div className="relative overflow-hidden h-auto">
          ...
          <form
            ...
            className={`transition-all duration-500 ${
              isLogin ? "opacity-100" : "opacity-0 -translate-x-full absolute"
            }`}
          >
            <FormInput
              id="loginEmail"
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full p-3.5 bg-[#00ff85] text-black font-bold uppercase tracking-wider rounded-lg hover:transform hover:-translate-y-1 hover:shadow-lg hover:shadow-[#00ff85]/30 transition-all disabled:opacity-70 disabled:cursor-not-allowed"
            >
